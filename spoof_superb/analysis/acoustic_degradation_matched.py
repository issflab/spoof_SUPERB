"""Acoustic degradation with composition AND coverage matched (Section 5.2).
    ...
    python -m spoof_superb.analysis.acoustic_degradation_matched \
        --out_dir outputs/degradation_matched

Why this exists alongside `acoustic_degradation`
------------------------------------------------
The published matrix pools each condition from partitions of four corpora and
keeps the corpora it does not degrade. Two things then vary between the
Baseline and a degraded condition, on top of the degradation itself:
...
def main(argv=None):
    ap = argparse.ArgumentParser(
        prog="python -m spoof_superb.analysis.acoustic_degradation_matched",
        description=__doc__,
