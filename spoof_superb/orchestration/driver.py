...
Three defaults worth knowing:
  ...
  * **No verification happens, and none can be switched on here.** Scoring
    never reads a score file it did not just write, so a fresh tree is built
    from protocols alone and cannot inherit an older tree's coverage. There is
    deliberately no `--verify-against`: a comparison that runs inside the sweep
    grades a tree against whatever happened to be on disk at the time, buries
    the verdict in a run status file nobody opens again, and makes the sweep's
    exit code depend on a second tree. Verification is its own step, with its
    own report:
...
def run_task(job, task, gpu, python, force=False, reporter=None, slot=None):
    ...
    # displayed as live progress for a task that is only being counted.
    ...
    if resume:
        reporter.write(f"[orchestrate] {task.name}: existing output is "
                       f"complete; skipping")
        rc = 0
    else:
    ...
    # This reads the whole score file back; on ASVLD that is minutes, and the
    # display would otherwise show a slot frozen at 100%.
    ...
    reporter.write(f"{rec['status'].upper():8s} {task.name}: {n:,} lines, "
                   f"{n_bona:,} bonafide, {n_spoof:,} spoof, {n_nan} NaN")
...
def write_summary(job, tasks):
    ...
    with open(path, "w") as f:
        ...
        f.write(f"{'task':40s} {'status':9s} {'lines':>10s} "
                f"{'bonafide':>10s} {'spoof':>10s} {'NaN':>8s}\n")
        for t in tasks:
            r = _results.get(t.name, {})
            f.write(f"{t.name:40s} {r.get('status', '?'):9s} "
                    f"{r.get('n_lines', 0):>10} {r.get('n_bonafide', 0):>10} "
                    f"{r.get('n_spoof', 0):>10} {r.get('n_nan', 0):>8}\n")
    print(f"\nSummary -> {path}")
